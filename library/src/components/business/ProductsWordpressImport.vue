<template>
  <div v-loading.fullscreen="loading">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div>
        <button v-if="enableImport" class="btn btn-primary white font-14 mr-2" type="button" @click="showPreviewWordpress()">Import</button>
    ...
    <el-dialog title="Preview imported products" :close-on-click-modal="false" :visible.sync="visible" width="95%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="importProducts()" :disabled="uploading">{{ uploading ? "Importing...": "Import" }}</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  created() {
    ...
    axios.get('/api/wordpress/retrieve-products/').then(function (res) {
  ...
  methods: {
    showPop() {
      if (confirm("You didn't connect to Shopify. Do you want to connect now?")) {
        window.location.href = '/users/wordpress-integration/';
      }
    },
    showPreviewWordpress() {
      ...
      if (confirm("Are you sure?")) {
        ...
        this.selected_products.forEach(p => {
          requests.push(limit(() => new Promise((resolve, reject) => {
            ...
            axios.post("/api/wordpress/import-product/", p).then((res) => {
